...
async function main() {
  ...
  const passwordAdmin = await hash("Admin1234", ARGON2);
  const passwordVende = await hash("Vende1234", ARGON2);
  ...
  try {
    ...
    const adminId = await inserta(
      `INSERT INTO usuarios
       ...
       VALUES ($1, NULL, $2, $3, false, $4, $5, 'SUPERADMIN')
       RETURNING id`,
    ...
    const scAdmin = await inserta(
      `INSERT INTO usuarios
         (id, empresa_id, username, password_hash, debe_cambiar_password,
          nombres, apellidos, rol)
       VALUES ($1, $2, $3, $4, false, $5, $6, 'ADMIN_EMPRESA')
       RETURNING id`,
    ...
    const scVende1 = await inserta(
      `INSERT INTO usuarios
       ...
       VALUES ($1, $2, $3, $4, false, $5, $6, 'VENDEDOR', $7)
       RETURNING id`,
      [
        randomUUID(),
        scId,
    ...
    console.log("✓ Seed aplicado (id admin:", adminId + ")");
  } catch (err) {
